...
case class Longitudinal() extends DerivedStream {
  ...
  private def buildRecord(history: Iterable[Map[String, Any]], schema: Schema): Option[GenericRecord] = {
    ...
    val unique = history.foldLeft((List[Map[String, Any]](), Set[String]()))(
      { case ((submissions, seen), current) =>
        ...
        val docId = current.getOrElse("documentId", {
          output("==========================================DOCID MISSING\n")
          return None
        }).asInstanceOf[String]
    ...
    val sorted = unique.flatMap{x =>
      val info = parse(x.getOrElse("payload.info", {
        output("==========================================INFO MISSING\n")
        return None
      }).asInstanceOf[String])
  ...
  def output(value: String) = {
    val fw = new java.io.FileWriter("/home/hadoop/OUTPUT.txt", true)
    try { fw.write(value) }
    finally fw.close()
  }
}
